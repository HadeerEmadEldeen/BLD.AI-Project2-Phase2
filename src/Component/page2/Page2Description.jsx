...
function Page2Description(props) {
  ...
  let total = "";
  function showmore() {
    let descriptionId = document.getElementsByClassName("showmore2");
    if (btnstate == false) {
      for (let i = 0; i < descriptionId.length; i++) {
        descriptionId[i].style.display = "block";
      }
    } else {
      for (let i = 0; i < descriptionId.length; i++) {
        descriptionId[i].style.display = "none";
      }
    }
  ...
  total = props.course.content.map((it) => {
    ...
    return <p className={classs}>{it}</p>;
  });
  let total2 = (
    <div className="showmore2">
  ...
  return (
    <section>
      ...
      <div className="description-details">{total}</div>
      {total2}
      {butt}
    </section>
  );
